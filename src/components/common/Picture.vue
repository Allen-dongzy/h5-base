...
const emit = defineEmits(['update:state', 'delete', 'select'])
...
<template>
  <div class="picture">
    ...
    <!-- 将upload模块改为自定义插槽,如果没有才默认显示下面的模块 -->
    <slot v-if="props.mode === 'single' && !pictureState.url">
      <div
        class="upload flex flex-col jc-center ai-center"
        :style="{ width: `${pictureState.width}px`, height: `${pictureState.height}px` }"
        @click="selectPicture"
      >
